...
def get_test_data():
    initial_df = pd.read_csv(r'C:\Users\Brandon\PycharmProjects\ProductPricePredictionProject\Data\knn_test_data.csv',
                             usecols=['Product Name', 'ASIN', 'Keepa Link', 'Rating', '# of Reviews', 'Department',
                                      'Department Tree', 'Pricing Pattern'])

    df = initial_df.loc[initial_df["Pricing Pattern"] != 'BAD']
...
def run():
    ...
    df = assign_category_values(initial_df)
    return df
